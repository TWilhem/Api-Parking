...
setup_cron_in_docker() {
    ...
    if [[ "$Crontab" != "disable" ]]; then
        ...
        docker exec "$DOCKER_CONTAINER" bash -c "echo '$cron_interval root cd /var/www/html && /opt/venv/bin/python main.py >> /var/log/cron.log 2>&1' > /etc/cron.d/Parking-Crontab"
